			...
			savedData.tileValues.forEach((tileValue) => {
				if (tileValue.cellId) {
					...
					if (tileValue.arrivalTime) {
						const arrInput = document.getElementById(`arrival-time-${tileValue.cellId}`);
						if (arrInput) {
					...
					if (tileValue.departureTime) {
						const depInput = document.getElementById(`departure-time-${tileValue.cellId}`);
						if (depInput) {
					...
					if (tileValue.aircraftId) {
						const aircraftInput = document.getElementById(`aircraft-${tileValue.cellId}`);
						if (aircraftInput) {
					...
					// Notes
					if (tileValue.notes) {
						const notesInput = document.getElementById(`notes-${tileValue.cellId}`);
						if (notesInput) {
...
function saveFlightTimeValueToLocalStorage(cellId, field, value) {
	try {
		...
		if (window.hangarEventManager && window.hangarEventManager.saveToStorage) {
			window.hangarEventManager.saveToStorage("hangarPlannerSettings", settings, "high");
		} else {
...
function updateStatusLights(cellId) {
	...
	const statusLight = document.querySelector(`.status-light[data-cell="${cellId}"]`);

	if (statusSelect && statusLight) {
		const statusValue = statusSelect.value;
		
		// Alle Status-Klassen entfernen
		statusLight.classList.remove("status-ready", "status-maintenance", "status-aog");
...
function searchAircraft() {
	...
	if (!found) {
		...
		if (window.showNotification) {
			window.showNotification(`Flugzeug "${searchTerm}" nicht gefunden`, "warning");
		}
	} else {
...
function fetchAndUpdateFlightData() {
	...
	if (window.FlightDataAPI && typeof window.FlightDataAPI.fetchFlightData === 'function') {
		const params = {
			currentDate,
			nextDate,
			airportCode,
...
function applyFlightDataToUI(flightData) {
	...
	flightData.forEach((flight, index) => {
		...
		if (cellId <= 12) { // Nur auf verfügbare Kacheln anwenden
			// Aircraft ID
			...
			if (aircraftInput && flight.aircraftId) {
				...
				saveFlightTimeValueToLocalStorage(cellId, 'aircraftId', flight.aircraftId);
			}
			...
			if (arrivalInput && flight.arrivalTime) {
				...
				saveFlightTimeValueToLocalStorage(cellId, 'arrivalTime', flight.arrivalTime);
			}
			...
			if (departureInput && flight.departureTime) {
				...
				saveFlightTimeValueToLocalStorage(cellId, 'departureTime', flight.departureTime);
			}
		}
	});
