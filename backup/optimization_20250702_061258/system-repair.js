...
window.SystemRepair = {
	...
	consolidateCollectAllHangarData() {
		...
		if (
			...
			console.log(
				"ℹ️ collectAllHangarData: Bestehende Implementierung beibehalten"
			);
		} else {
			// Fallback erstellen
	...
	consolidateSaveFlightTimeValue() {
		...
		if (
			window.hangarEvents &&
			typeof window.hangarEvents.saveFlightTimeValueToLocalStorage ===
				"function"
		) {
		...
		if (
			typeof window.saveFlightTimeValueToLocalStorage === "function" &&
		...
		if (implementations.length > 1) {
			console.log(
				`⚠️ Mehrfache Implementierungen von saveFlightTimeValueToLocalStorage: ${implementations.join(
					", "
				)}`
			);
			...
			if (
				...
				window.saveFlightTimeValueToLocalStorage =
					window.hangarEvents.saveFlightTimeValueToLocalStorage;
				console.log(
					"✅ saveFlightTimeValueToLocalStorage: hangar-events als Master gesetzt"
				);
			}
		}
	},
	...
	consolidateEventManagers() {
		...
		if (managers.length > 1) {
			console.log(
				`⚠️ Mehrfache Event Manager gefunden: ${managers.join(", ")}`
			);
	...
	determineLatestVersion(localData, serverData) {
		...
		const localTime =
			...
			localData.metadata?.exportDate ||
			"1970-01-01";
		const serverTime =
			serverData.metadata?.lastModified ||
			serverData.metadata?.timestamp ||
			"1970-01-01";
	...
	async syncToServer(data) {
		if (
			...
			typeof window.displayOptions.saveToServer === "function"
		) {
			await window.displayOptions.saveToServer();
		}
	},
