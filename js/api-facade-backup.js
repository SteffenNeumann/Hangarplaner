...
const FlightDataAPI = (function () {
	...
	const updateAircraftData = async function (
		...
		try {
			...
			if (config.activeProvider === "flightradar24") {
				return await handleFlightradar24Request(aircraftId, currentDate, nextDate);
			} else {
				// Standard: AeroDataBox
				return await handleAeroDataBoxRequest(aircraftId, currentDate, nextDate);
			}
	...
	/**
	 * Behandelt AeroDataBox API-Anfragen
	 */
	const handleAeroDataBoxRequest = async function(aircraftId, currentDate, nextDate) {
		...
		if (!originalResult) {
			console.log(
				"[API-FASSADE] Keine Ergebnisse von der AeroDataBox API erhalten, gebe leere Werte zurück"
			);
			return {
				originCode: "",
				destCode: "",
