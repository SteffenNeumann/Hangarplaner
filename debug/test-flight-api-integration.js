...
async function testMockAPICall() {
	...
	try {
		...
		if (window.HangarData && window.HangarData.updateAircraftFromFlightData) {
			...
			const firstAircraftInput = document.querySelector(
				'input[id^="aircraft-"]'
			);
...
async function runAllTests() {
	...
	results.push({
		name: "Automatische Datumseintragung",
		passed: testAutomaticDateSetup(),
	});
	results.push({
		name: "API-Fassade Verfügbarkeit",
		passed: testAPIFacadeAvailability(),
	});
	results.push({
		name: "HangarData Update-Funktion",
		passed: testHangarDataUpdateFunction(),
	});
	results.push({
		name: "Event-Handler Update-Button",
		passed: testUpdateButtonEventHandler(),
	});
	...
	if (passedTests === totalTests) {
		console.log(
			"🎉 Alle Tests erfolgreich! Die Flugdaten-API-Integration funktioniert korrekt."
		);
	} else {
		console.log(
			"⚠️ Einige Tests fehlgeschlagen. Bitte prüfen Sie die Implementierung."
		);
	}
