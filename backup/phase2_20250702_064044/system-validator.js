	...
	async validateFunctionConflicts() {
		...
		this.testFunction("Event Manager Conflicts", () => {
			...
			if (eventManagers.length > 1) {
				this.addWarning(
					`Mehrfache Event-Manager gefunden: ${eventManagers.length}`
				);
			}
	...
	async validateServerSync() {
		...
		this.testFunction("Data Format Validation", async () => {
			try {
				...
				this.addWarning(
					`Server-Datenformat prüfung fehlgeschlagen: ${error.message}`
				);
				return true;
			}
		});
			...
			numberInputs.forEach((input) => {
				if (input.min && input.max) {
					...
					if (value < min || value > max) {
						this.addWarning(
							`Eingabefeld ${input.id} außerhalb der Grenzen: ${value} (${min}-${max})`
						);
					}
				}
			});
